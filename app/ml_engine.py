...
try:
    ...
    _gru_model.load_state_dict(
        torch.load(f"{MODEL_DIR}/gru_model.pt", map_location=torch.device("cpu"))
    )
    _gru_model.eval()
...
def score_from_kafka(record: dict) -> tuple:
    ...
    print(
        f"[ML DEBUG] GRU History (last row): {history[-1] if len(history) > 0 else 'EMPTY'}"
    )
    ...
    for s in merged_shap:
        print(
            f"  - {s['feature']}: contrib={s['contribution']:.4f}, dir={s['direction']}, val={s['value']}"
        )
...
def get_risk_level(score: float) -> str:
    # Use 0.50 as the base threshold for voice intervention
    thr = 0.50
